...
for(sam in samples)
{
    ...
    d=rbind(d,tmp[2,])
    tryCatch({tmp=as.matrix(read.table(sprintf('../results/%s_perfMTL',sam),sep=','))},warning=function(e) tmp=matrix(rep(NA,14),2,7)) 
    d=rbind(d,tmp[2,])
    tryCatch({tmp=as.matrix(read.table(sprintf('../results/%s_perfPairEns',sam),sep=','))},warning=function(e) tmp=matrix(rep(NA,35),5,7)) 
    d=rbind(d,tmp[-1,])
    tryCatch({tmp=as.matrix(read.table(sprintf('../results/%s_perfTreeEns',sam),sep=','))},warning=function(e) tmp=matrix(rep(NA,35),5,7)) 
    d=rbind(d,tmp[-1,])
